...
- **Why the `rb` step.** It splits each chunk between the two ends of `b`, so the shorter rotation direction is on average about half as long. It is a constant-factor optimisation and does not change the bound. On the permutations behind the *Measured performance* section below, leaving it out raises the average from 785 to 811 operations (n = 100) and from 7378 to 8049 (n = 500). With it the result is cheaper on 95 of the 100 permutations at n = 100 and on all 20 at n = 500.
- **Space.** O(1) auxiliary memory.
